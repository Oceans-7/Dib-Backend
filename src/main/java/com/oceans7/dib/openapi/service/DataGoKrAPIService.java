...
import com.oceans7.dib.openapi.DataGoKrApi;
...
public class DataGoKrAPIService extends OpenAPIService {
    ...
    public DetailIntroResponse getIntroApi(Long contentId, String contentTypeId) {
        ...
        switch(ContentType.getContentTypeByCode(Integer.parseInt(contentTypeId))) {
            case TOURIST_SPOT -> { return parsingJsonObject(result, SpotIntroResponse.class); }
            case CULTURAL_SITE -> { return parsingJsonObject(result, CultureIntroResponse.class); }
            case EVENT -> { return parsingJsonObject(result, EventIntroResponse.class); }
            case LEPORTS -> { return parsingJsonObject(result, LeportsIntroResponse.class); }
            case ACCOMMODATION -> { return parsingJsonObject(result, AccommodationIntroResponse.class); }
            case SHOPPING -> { return parsingJsonObject(result, ShoppingIntroResponse.class); }
            case RESTAURANT -> { return parsingJsonObject(result, RestaurantIntroResponse.class); }
            default -> { return null; }
        }
    }
