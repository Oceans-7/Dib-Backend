...
public class WeatherController {
    ...
    @Operation(summary = "위치 기반 날씨 현재 정보 조회", description = "위도와 경도를 받아 해당 지역의 현재 날씨 정보를 조회한다.")
    @GetMapping("/current")
    @ApiResponses(value = {
    ...
    public ApplicationResponse<GetCurrentWeatherResponseDto> getWeather(
        ...
        GetCurrentWeatherResponseDto weather = weatherService.getCurrentWeather(getLocationWeatherRequestDto);
        return ApplicationResponse.ok(weather);
    }

    @Operation(summary = "위치 기반 날씨 일별 정보 조회", description = "위도와 경도를 받아 해당 지역의 일별 날씨 정보를 조회한다.")
    @GetMapping("/daily")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공"),
            @ApiResponse(responseCode = "W0000", description = "날씨 정보를 찾을 수 없습니다.", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    ...
    public ApplicationResponse<GetForeCastWeatherResponseDto> getForecastWeather(
            @ParameterObject @ModelAttribute @Validated GetLocationWeatherRequestDto getLocationWeatherRequestDto
    ) {
        GetForeCastWeatherResponseDto weather = weatherService.getForecastWeather(getLocationWeatherRequestDto);
        return ApplicationResponse.ok(weather);
    }
}
